...
STUDENT_ID_PATTERN = re.compile(r"^S\d{8}$")
ALLOWED_EXTENSIONS = {".pdf"}
MIN_PAGE_COUNT = 15
MAX_TITLE_LENGTH = 180


def load_records(csv_path: Path) -> list[ThesisRecord]:
        ...
        for row in reader:
            records.append(
                ThesisRecord(
                    ...
                    degree_program=row["degree_program"].strip(),
                    submission_term=row["submission_term"].strip(),
...
def build_output_filename(record: ThesisRecord) -> str:
    """Build normalized output filename used by the archive convention."""
    ...
    return f"{record.submission_term}_{record.student_id}_{safe_author}_{safe_title}.pdf"


def validate_record(record: ThesisRecord) -> ValidationResult:
    """Run format and metadata validations on one thesis record."""
    errors: list[str] = []

    if not STUDENT_ID_PATTERN.match(record.student_id):
        errors.append("Student ID must match pattern S########.")
...
def process_records(records: list[ThesisRecord]) -> list[ValidationResult]:
